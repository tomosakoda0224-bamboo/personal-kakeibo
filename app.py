...
def initialize_sheets():
    ...
    if not categories.row_values(1):
        ...
        categories.append_rows([[name, icon] for name, icon in DEFAULT_CATEGORIES.items()])
...
def load_categories(category_sheet):
    ...
    result = {
        str(row.get("カテゴリー", "")).strip(): str(row.get("アイコン", "🏷️")).strip()
        for row in rows
...
def load_expenses(expense_sheet, start, end):
    ...
    frame["金額"] = pd.to_numeric(frame["金額"], errors="coerce").fillna(0).astype(int)
    frame = frame[
        frame["日付"].between(pd.Timestamp(start), pd.Timestamp(end), inclusive="both")
    ].copy()
...
def render_calendar(start, end, daily_amounts):
    ...
    for day in days:
        ...
        background = (
            f"rgba(242,111,82,{0.08 + intensity * 0.55:.2f})" if amount else "#f7f7f3"
        )
        amount_html = f"<strong>¥{amount:,}</strong>" if amount else "<span>—</span>"
...
st.markdown(
    ...
    .hero{background:linear-gradient(135deg,#244b3b,#386c57);color:white;
      padding:28px 32px;border-radius:24px;margin-bottom:22px;
      box-shadow:0 12px 34px rgba(36,75,59,.16)}
    .hero h1{color:white;margin:0;font-size:2rem}.hero p{margin:.4rem 0 0;opacity:.8}
    div[data-testid="stForm"],div[data-testid="stVerticalBlockBorderWrapper"]{
      background:#fff;border-radius:20px;border-color:#e7e1d8}
    div[data-testid="stMetric"]{background:white;padding:16px 20px;border-radius:18px;
      border:1px solid #e7e1d8}
    .calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(75px,1fr));gap:8px}
    .cal-cell{min-height:74px;border-radius:13px;padding:10px;display:flex;
      flex-direction:column;justify-content:space-between;border:1px solid rgba(36,56,47,.06)}
    .cal-cell small{color:#52645c;font-weight:700}.cal-cell strong{color:#7c2f1f;font-size:.9rem}
...
try:
    ...
    st.info(
        "`.streamlit/secrets.toml` の設定と、スプレッドシートがサービスアカウントに"
...
with entry_tab:
    with st.form("expense_form", clear_on_submit=True, border=True):
        ...
        with year_col:
            year = st.selectbox("年", range(today.year - 10, today.year + 2), index=10)
        with month_col:
            month = st.selectbox("月", range(1, 13), index=today.month - 1)
        max_day = calendar.monthrange(year, month)[1]
        with day_col:
            day = st.selectbox("日", range(1, max_day + 1), index=min(today.day, max_day) - 1)
        ...
        selected_label = st.radio("カテゴリー", labels, horizontal=True)
        submitted = st.form_submit_button("スプレッドシートに保存", type="primary", use_container_width=True)
        if submitted:
            ...
            if not item.strip():
                ...
                st.success(f"{item.strip()}（{money(cleaned)}）を保存しました。")

    with st.expander("➕ カテゴリーを追加"):
        with st.form("category_form", clear_on_submit=True):
            icon_col, name_col = st.columns([1, 3])
            with icon_col:
                new_icon = st.text_input("アイコン", value="🏷️", max_chars=4)
            with name_col:
                new_name = st.text_input("カテゴリー名", placeholder="例：医療")
            if st.form_submit_button("カテゴリーを追加"):
                if not new_name.strip():
                    st.error("カテゴリー名を入力してください。")
                elif new_name.strip() in categories:
                    st.warning("同じ名前のカテゴリーがすでにあります。")
                else:
                    category_sheet.append_row([new_name.strip(), new_icon.strip() or "🏷️"])
                    st.success(f"{new_icon or '🏷️'} {new_name.strip()} を追加しました。")
                    st.rerun()
...
with report_tab:
    ...
    start, end = shift_period(current_start, st.session_state.period_offset)
    ...
    with title:
        st.markdown(
            f"<h3 style='text-align:center;margin:.35rem 0'>{start:%Y年%m月%d日} 〜 {end:%Y年%m月%d日}</h3>",
            unsafe_allow_html=True,
        )
    with following:
    ...
    days_used = int(expenses["日付"].nunique()) if not expenses.empty else 0
    metric1, metric2, metric3 = st.columns(3)
    metric1.metric("期間の合計金額", money(total))
    metric2.metric("支出があった日", f"{days_used} 日")
    metric3.metric("使用日の平均", money(round(total / days_used) if days_used else 0))

    st.subheader("日ごとの使用金額")
    st.caption("色が濃い日ほど、使用金額が多い日です。緑の枠は今日を表します。")
